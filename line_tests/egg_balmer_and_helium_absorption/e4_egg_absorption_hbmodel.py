#!/usr/bin/env python3
"""e4_egg_absorption + anchored-model overlay on the Hbeta panel. DISPLAY TEST,
not the paper figure (canonical e4_egg_absorption.pdf untouched).

Adds to the Hbeta panel the definitive anchored joint Hb+[OIII] decomposition
(gauss family canon, definitive_egg_gauss_params.npy), self-normalized by its
own fitted continuum: total model and the same model with the absorber zeroed,
so the -206 km/s deficit is the gap between the two curves.

Two variants: A = exact paper panel (Hbeta from MODS-R, as in e4);
B = same figure with the Hbeta panel data swapped to MODS-B, the arm the
decomposition actually fit (5352 A is MODS-R's bad blue edge but MODS-B's
good red end).
"""
import os
import numpy as np
...
def make(hb_data, hb_lab, outstem):
    LINES = [(r'H$\alpha$ 6563', 6564.61, mr), (hb_lab, 4862.68, hb_data),
    ...
    for k, (ax, (lab, l0, (w, f, s))) in enumerate(zip(axes, LINES)):
        ...
        if k == 1:   # Hbeta panel: anchored-model overlay
            ...
            ax.plot(vg, noabs, color='#228833', lw=1.2, ls='--', alpha=0.9,
                    zorder=4, label='model, absorber removed')
            ...
            # zoom inset on the blue flank where the absorber lives
            axi = ax.inset_axes([0.055, 0.10, 0.36, 0.46])
            ...
            axi.set_xlim(-500, 120)
            axi.set_ylim(0.7, 3.4)
            axi.tick_params(which='both', direction='in', labelsize=5.5,
                            length=2.5)
...
vg = np.array([dva])
tot, noabs = hb_model_norm(vg, 4862.68)
print('model absorber: dv=%.1f km/s  sigma=%.1f km/s  depth=%.3f of continuum'
      % (dva, sa, float(noabs[0] - tot[0])))
make(mr, r'H$\beta$ 4861 (MODS-R)', 'e4_egg_absorption_hbmodel')
make(mb, r'H$\beta$ 4861 (MODS-B)', 'e4_egg_absorption_hbmodelB')
