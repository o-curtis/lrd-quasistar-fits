...
CASES = {
    ...
    'B one absorber, v free   ': (head + [(-400, 250), (40, 160), (-2*aB, 0)] + tail, False),
    'C two absorbers          ': (head + [(-400, -40), (40, 160), (-2*aB, 0)]
                                       + [(-40, 250), (40, 200), (-2*aB, 0)] + tail, True),
}
...
a, c = out['A published (blue pinned)'], out['C two absorbers          ']
b = out['B one absorber, v free   ']
print('N points = %d, lnN = %.3f' % (N, lnN))
print('B - A : dchi2 = %+8.1f (same k)   dBIC = %+8.1f' % (b['chi2']-a['chi2'], b['bic']-a['bic']))
print('C - A : dchi2 = %+8.1f (3 extra)  dBIC = %+8.1f' % (c['chi2']-a['chi2'], c['bic']-a['bic']))
json.dump(out, open('/home/omc5226/work/lrdmesa/gobig_work/egg_hb_absorber2.json', 'w'), indent=1)
print('\nwrote egg_hb_absorber2.json')
