#!/usr/bin/env python3
"""
fit_capers_m3.py — water dot CAPERS-UDS-23216 (z=2.3203), adopted Egg M3 port.
...
READY TO SUBMIT — do NOT auto-run.
    python fit_capers_m3.py --pool N
    python fit_capers_m3.py --dry-run
"""
import os, argparse
import numpy as np
...
FITS  = os.path.join(ROOT, 'lrd_spectra/wide_egs_2974_prism.fits')
Z     = 2.3203
LABEL = 'CAPERS-UDS-23216'
STEM = 'CAPERS_m3b_r300_ujy'             # Plan A + Calzetti dust ([M/H]=-1 fixed, A_V<0.5, dust1=0)
...
WIDE_EXTRA  = [(3101,45)]            # ~0.31 um feature, WIDE only
...
def build_obs():
    with fits.open(FITS) as h:
        ...
        flam  = d['flux'].astype(float) * 1e-29 * 2.998e18 / w_obs**2   # DJA uJy -> f_lambda
