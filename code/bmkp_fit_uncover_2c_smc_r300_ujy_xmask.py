...
import fit_uncover_m3_r300 as base

STEM = 'BMKP_UNCOVER_2c_smc_r300_ujy_xmask'
...
def build_obs():
    ...
    for arm in arms:
        ...
        # XMASK (2026-09-15): also drop the chi2 drivers flagged by chi2_drivers.py: the single -5 sigma pixel near 4562 A
        # and the +3 sigma cluster at 3230-3380 A (Fe II / Balmer-continuum region), to test whether they pull the solution
        w = arm['wave_rest']
        xm = ((w >= 4540.0) & (w <= 4585.0)) | ((w >= 3230.0) & (w <= 3380.0))
        print('XMASK removed %d more pixels' % int((arm['mask'] & xm).sum()), flush=True)
        arm['mask'] = arm['mask'] & ~xm
