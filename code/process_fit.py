#!/usr/bin/env python3
"""Process a completed campaign fit: posterior phi/T/logg + spectrum overlay.
Uses the posterior-median theta for the overlay (robust; no MAP loop)."""
import os, sys, numpy as np
import m3port as M
import matplotlib; matplotlib.use("Agg"); import matplotlib.pyplot as plt

STEM = sys.argv[1]; DRIVER = sys.argv[2]
CAMP = "/storage/group/jtw13/default/oCurtisWorkDir/lrdmesa/blueshift-phi-campaign"
drv = __import__(DRIVER)
chain = np.load(os.path.join(CAMP, "chain_%s.npy" % STEM))
sps = M.build_sps(dust_law=drv.DUST_LAW, verbose=False)
model = M.build_model(drv.Z, **drv.MODEL_KWARGS)
obs = drv.build_obs(); idx = model.theta_index
...
print("RESULT %s  N=%d" % (STEM, len(chain)))
print("  Teff = %.0f (+%.0f/-%.0f) K" % (tq[1], tq[2]-tq[1], tq[1]-tq[0]))
print("  logg = %.2f (+%.2f/-%.2f)" % (gq[1], gq[2]-gq[1], gq[1]-gq[0]))
print("  logL = %.2f   A_V = %.2f" % (Lq[1], Aq[1]))
print("  phi  = %.1f (+%.1f/-%.1f)" % (pq[1], pq[2]-pq[1], pq[1]-pq[0]))
if "MOCK" in STEM and os.path.exists(os.path.join(CAMP, "mock_cliff.npz")):
    ...
    print("  TRUTH teff=%.0f logg=%.2f phi=%.1f | logg recovered within %.1f sigma"
          % (z["teff_true"], z["logg_true"], z["phi_true"], dg))
# median-theta overlay
th = np.array([np.median(chain[:, j]) for j in range(chain.shape[1])])
o = obs[0]; ptl, pgal = M.predict_components(th, sps, obs, model)[0]
rng = np.random.default_rng(1)
draws = chain[rng.choice(len(chain), min(100, len(chain)), replace=False)]
band = []
for d in draws:
    try:
