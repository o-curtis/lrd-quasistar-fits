...
STEM  = 'UNCOVER_m3b_r300_photcal'           # Plan A + Calzetti dust ([M/H]=-1 fixed, A_V<0.5, dust1=0)
...
def build_obs():
    ...
    # PHOTCAL (2026-09-15): re-anchor the DJA v4 spectrum to the UNCOVER DR2 NIRCam photometry (Weaver+2024):
    # c(lambda_obs) = 0.998 + 0.168*(lambda_um - 2.76) for lambda >= 2.76 um, else 0.998 (weighted fit through the
    # catalog/spectrum flux ratios in F277W, F356W, F410M, F444W; blue bands left uncorrected = extended host light).
    _lam_um = w_obs * 1e-4
    _c = np.where(_lam_um >= 2.76, 0.998 + 0.168 * (_lam_um - 2.76), 0.998)
