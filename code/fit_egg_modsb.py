#!/usr/bin/env python3
"""
Fit The Egg (J1025+1402) using MODS Blue spectrum alone.
1-component TLUSTY hot atmosphere + AGN power-law, tested with both
Calzetti and SMC extinction laws.

Fit window: 3000-5900 Å rest-frame.
Key diagnostics: Wien peak of T~5744 K (peak ~5050 Å), Ca H&K (3934/3968 Å),
Balmer lines (Hδ 4102, Hγ 4340 — Hβ masked due to broad AGN emission).
No cool component — T~2400 K contributes <0.1% at these wavelengths.

Outputs:
  egg_chain_modsb_calzetti.npy / .pkl
  egg_chain_modsb_smc.npy / .pkl
  egg_bic_modsb.txt
"""
...
print(f"  {len(model_meta)} models loaded", flush=True)
...
modsb_path = os.path.join(DATA_DIR, 'J1025+1402_MODSB_coadd1d_tellcorr_slitcorr_dered.fits')
with fits.open(modsb_path) as h:
    s = h['SPECTRUM'].data
...
# ── Emission line masks (optical — heavy broad-line masking) ─────────────────
LINE_MASKS_REST = [
    (4861, 100),   # Hβ broad BLR — wide mask ±6000 km/s
    (4340,  50),   # Hγ partial BLR
    (4102,  30),   # Hδ — narrower (less BLR contamination)
...
# Upweight Ca H&K (gravity diagnostic) and Wien peak region
CAK_BAND  = (w_cen >= 3850) & (w_cen <= 4000)   # Ca H&K
WIEN_BAND = (w_cen >= 4500) & (w_cen <= 5500)    # Wien peak of T~5744 K
...
print(f"Fit window: {FIT_LO:.0f}–{FIT_HI:.0f} Å | {n_pix} unmasked pixels", flush=True)
...
if __name__ == '__main__':
    samp_calz, res_calz = run_fit(calzetti_ext, 'MODS Blue — Calzetti', 'egg_chain_modsb_calzetti')
    samp_smc,  res_smc  = run_fit(smc_ext,      'MODS Blue — SMC',      'egg_chain_modsb_smc')

    n = n_pix; k = 5
    lines = [
        f"The Egg (J1025+1402) — MODS Blue Only Fit",
        f"Fit window: {FIT_LO:.0f}–{FIT_HI:.0f} Å rest | n_pix = {n}",
        f"Model: 1-comp TLUSTY + AGN PL | ndim=5",
        f"NOTE: Hβ broad masked ±100 Å; Hγ masked ±50 Å; Hδ masked ±30 Å",
        f"NOTE: No cool component (negligible at <6000 Å for T_cool~2400 K)",
        f"",
    ]
    ...
    report = '\n'.join(lines)
    print(report)
    with open(os.path.join(SCRIPT_DIR, 'egg_bic_modsb.txt'), 'w') as fh:
        fh.write(report)
    print("Done.")
